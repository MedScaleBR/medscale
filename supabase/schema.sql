...
drop table if exists
  ...
  public.finance_projections,
  public.finance_investments,
  public.finance_reserve_movements,
  public.finance_reserves,
...
drop function if exists public.enforce_reserve_movement_account() cascade;
...
-- Árvore de categorias/subcategorias do financeiro (Bloco A). Por account e
-- por kind (pf/pj). parent_id null = categoria-raiz; preenchido = subcategoria.
-- Profundidade máxima 2 níveis, garantida por trg_enforce_finance_category_depth.
-- Definida antes de finance_entries porque este referencia finance_categories.
create table public.finance_categories (
  ...
  -- Gasto excessivo só vira sugestão em categoria NÃO essencial (o que é
  -- supérfluo para um médico pode ser indispensável para outro). Default true
  -- para não disparar alerta retroativo em categoria que o owner nunca
  -- revisou; ele desmarca as que considera supérfluas. Só faz diferença em
  -- direction = 'out'. Ver lib/finance/suggestions.ts.
...
-- índice único idx_finance_categories_unique_sibling (seção 9). translate() em
-- vez de unaccent para não depender de extensão.
...
create table public.finance_entries (
  ...
  category          text,
  -- Categorização estruturada (Bloco A). NULL = sem categoria. subcategory_id,
  -- quando preenchido, tem que ser filha de category_id — coerência garantida
  -- pela trigger trg_enforce_finance_entry_category. FK on delete set null: a
  -- categoria pode ser removida sem apagar o lançamento.
...
-- Log interno das mensagens que chegam no número compartilhado do agente
-- financeiro. Não usa `conversations/messages` porque esse número não pertence
-- a uma workspace; ele roteia owner -> account antes de agir.
create table public.finance_agent_messages (
  id          uuid default uuid_generate_v4() primary key,
  account_id  uuid references public.accounts(id) on delete set null,
  phone       text not null,
...
-- ============================================================
-- 7C. PATRIMÔNIO (reservas, investimentos, projeções, metas, sugestões)
--     Extensão do módulo financeiro: o que o owner TEM guardado, o que
--     PRETENDE gastar e o que QUER juntar. Mesma regra de visibilidade do
--     resto do 7B — exclusivo do owner do account.
-- ============================================================

-- Caixinha nomeada de dinheiro guardado ("Reserva de emergência", "Viagem").
-- O saldo não é uma coluna: é a soma dos movimentos, para o histórico nunca
-- divergir do total.
create table public.finance_reserves (
  id          uuid default uuid_generate_v4() primary key,
  account_id  uuid references public.accounts(id) on delete cascade not null,
  -- Mesma divisão do resto do módulo. Default pf porque reserva é, na
  -- prática, patrimônio pessoal do médico.
  kind        text not null default 'pf' check (kind in ('pf','pj')),
  name        text not null,
  -- Arquivar em vez de apagar: os movimentos continuam valendo para o
...
create table public.finance_reserve_movements (
  ...
  -- garantida por trg_enforce_reserve_movement_account (seção 10.2).
  account_id  uuid references public.accounts(id) on delete cascade not null,
  amount      numeric(12,2) not null check (amount > 0),
  type        text not null check (type in ('deposit','withdrawal')),
  -- De onde veio. Espelha o sentinela recorded_by_phone = 'web'.
  source      text not null default 'web' check (source in ('web','whatsapp')),
  note        text,
  -- date, e não timestamptz: o módulo raciocina em dia (ver entry_date), o
  -- que evita bug de fuso em borda de mês.
  occurred_at date not null default current_date,
  created_at  timestamptz not null default now()
);

-- Aplicação financeira. Rendimento é DERIVADO na leitura por
-- lib/finance/investments.ts, nunca gravado: um valor estimado congelado numa
-- data passa a mentir no dia seguinte.
create table public.finance_investments (
  ...
  -- Valor atual INFORMADO pelo owner (obrigatório na prática para renda
  -- variável e cripto, onde não há taxa a projetar).
  current_value   numeric(12,2),
  -- Sem taxa, o rendimento não é calculado nem chutado: a tela mostra "dados
  -- insuficientes".
...
-- Quanto o owner planeja gastar numa categoria (ou subcategoria) no mês.
-- category_id e subcategory_id apontam AMBOS para finance_categories: a
-- árvore é uma tabela só, auto-referenciada. on delete cascade, e não
-- set null como em finance_entries: um lançamento sem categoria ainda é um
-- gasto que aconteceu, mas uma projeção sem categoria não significa nada.
create table public.finance_projections (
  ...
  -- Sempre o dia 1 do mês, normalizado por
  -- trg_normalize_finance_projection_period (seção 10.2).
...
create table public.finance_goals (
  ...
  -- owner quer ter guardados. 1 = um mês; 6 = reserva de emergência clássica.
  months_of_expenses numeric(5,2) not null default 1 check (months_of_expenses > 0),
  -- null = acompanha o patrimônio inteiro do kind (reservas ativas +
  -- investimentos). Preenchido = acompanha só aquela caixinha.
...
-- Alerta de gasto excessivo descartado pelo owner. Vale só até o fim do
-- period_month — no mês seguinte, se o padrão se repetir, o alerta volta.
...
-- TypeScript (DEFAULT_TOLERANCE em lib/finance/suggestions.ts), evitando um
-- insert de provisionamento em toda conta que nunca abriu a aba.
...
-- Impede duas caixinhas de mesmo nome no mesmo kind, o que tornaria o fuzzy
-- match do agente do WhatsApp ambíguo. Arquivadas entram no índice de
-- propósito: reusar o nome de uma arquivada confunde o histórico tanto quanto
-- duplicar uma ativa.
...
-- Coalesce pelo mesmo motivo de idx_finance_categories_unique_sibling: sem
-- ele, duas projeções da mesma categoria sem subcategoria passariam.
...
-- ============================================================
-- 10.2 TRIGGERS: integridade da árvore de finance_categories (Bloco A)
-- ============================================================
...
-- ------------------------------------------------------------
-- 10.3 TRIGGERS: integridade do patrimônio (seção 7C)
-- ------------------------------------------------------------

-- O movimento carrega account_id próprio (para a policy ser barata, sem
-- subquery por linha); esta trigger garante que ele nunca divirja do da
-- reserva. Roda BEFORE, então o preenchimento acontece antes do not null.
...
-- Coerência categoria/subcategoria nas duas tabelas que referenciam a árvore:
-- a subcategoria tem que ser filha da categoria, e ambas da mesma conta.
-- Mesmo contrato de enforce_finance_entry_category, generalizado via
-- tg_table_name para servir às duas.
...
-- finance_categories: árvore de categorias do painel financeiro — mesmo padrão
-- exclusivo do owner de finance_entries.
create policy "finance_categories: owner only" on public.finance_categories
  for all using (public.is_account_owner(account_id));

-- Patrimônio (seção 7C): mesma regra exclusiva do owner. O movimento de
-- reserva usa o próprio account_id (duplicado da reserva) para a policy não
-- fazer uma subquery por linha.
