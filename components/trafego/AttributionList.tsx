import { leadsByCampaign, type AttributedLead } from '@/lib/trafego/attribution'
...
export function AttributionList({ leads }: { leads: AttributedLead[] }) {
  const lines = leadsByCampaign(leads)
    ...
    <div className="rounded-[14px] border border-[var(--navy-06)] bg-white p-5 shadow-[var(--shadow-sm)]">
      ...
      {lines.length === 0 ? (
        ...
        <ul className="space-y-3">
          {lines.map((line) => {
            const canal = channelLabel(line.channel)
            const label = line.campaignName
              ? `${canal ? `${canal} · ` : ''}${line.campaignName}`
              : 'Campanha não identificada'

            return (
              <li
                key={line.campaignId ?? 'sem-campanha'}
                className="flex items-baseline justify-between gap-3"
              >
                <span className="truncate text-sm text-[var(--text-body)]" title={label}>
                  {label}
                </span>
